...
import { Memory, MemoryItem } from './MemoryItem';
...
export const MemoriesGallery: React.FC<MemoriesGalleryProps> = ({ circleId }) => {
    ...
    // 🚀 TURBO: Use a ref to track the currently playing audio instance.
    // This allows us to stop the previous audio before starting a new one, preventing overlap and memory leaks.
    ...
    // 🚀 TURBO: Stable callback for playing audio.
    // Passed to memoized MemoryItem, ensuring items don't re-render unless necessary.
    const handlePlay = useCallback((url: string) => {
        if (currentAudio.current) {
            currentAudio.current.pause();
            currentAudio.current.currentTime = 0; // Reset
        }
