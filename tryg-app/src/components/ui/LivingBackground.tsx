...
/**
 * LivingBackground 3.0 - The Circadian Engine
 * Renders the atmospheric background based on the 4 phases:
 * Dawn, Day, Golden, Midnight.
 */
export const LivingBackground: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const { circadianPhase } = useTheme();

    const theme = useMemo(() => {
        switch (circadianPhase) {
            case 'dawn':
                return {
                    gradient: 'from-[var(--dawn-from)] via-[var(--dawn-via)] to-[var(--dawn-to)]',
                    blob1: 'fill-purple-200/40',
                    blob2: 'fill-orange-100/30'
                };
            case 'day':
                return {
                    gradient: 'from-[var(--day-from)] via-[var(--day-via)] to-[var(--day-to)]',
            ...
            case 'golden':
                return {
                    ...
                    gradient: 'from-[var(--golden-from)] via-[var(--golden-via)] to-[var(--golden-to)]',
                    blob1: 'fill-amber-300/20',
                    blob2: 'fill-orange-200/30'
                };
            case 'midnight':
                return {
                    gradient: 'from-[var(--midnight-from)] via-[var(--midnight-via)] to-[var(--midnight-to)]',
    ...
    return (
        <div className={`h-full w-full bg-gradient-to-br transition-all duration-[4000ms] ease-in-out relative ${theme.gradient}`}>
            ...
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden blur-3xl opacity-80">
                <svg viewBox="0 0 100 100" className="absolute -top-10 -left-10 w-96 h-96 transition-colors duration-[4000ms]">
                    <path
                ...
                <svg viewBox="0 0 100 100" className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] transition-colors duration-[4000ms]">
                    <path
