    ...
    */
    ...
    return createPortal(
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
            <div className="theme-card w-full sm:max-w-md h-[85vh] sm:h-auto sm:max-h-[90vh] sm:rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl animate-slide-up border border-white/10">
                {/* Header */}
                <div className="px-6 py-6 border-b theme-header flex justify-between items-center opacity-90">
                    <div>
                        <h2 className="text-xl font-bold theme-text">{t('family_circle')}</h2>
                    </div>
                    <button
                        onClick={onClose}
                        className="p-2 hover:bg-stone-200 dark:hover:bg-slate-800 rounded-full transition-colors"
                    >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
                    ...
                    <section className="space-y-4">
                        <div className="flex items-center gap-2 text-stone-600">
                            ...
                            <h3 className="text-sm font-bold uppercase tracking-wider">LIVSTRÆET</h3>
                        </div>
                        ...
                        <div className="theme-bg rounded-3xl border border-stone-100 dark:border-slate-800 flex justify-center opacity-80 overflow-hidden">
                            <div className="scale-90 sm:scale-100 origin-center w-full">
                                <FamilyTree
                                    members={members}
                                    seniorId={members.find(m => m.role === 'senior')?.docId || ""}
                                    onUpdateMember={async (memberId, data) => {
                                        try {
                                            const memberRef = doc(db, 'careCircleMemberships', memberId);
                                            await setDoc(memberRef, data, { merge: true });
                                        } catch (e) {
                                            console.error("Error updating member from tree:", e);
                                            throw e;
                                        }
                                    }}
